...
if (!class_exists('WC_Gateway_Afterpay')) {
	class WC_Gateway_Afterpay extends WC_Payment_Gateway
	{
		...
		public function cart_is_within_limits() {
			...
			return
				$total >= $this->settings['pay-over-time-limit-min'] &&
				$total <= $this->settings['pay-over-time-limit-max'];
		}
			...
			if (!empty($_GET) && !empty($_GET['orderToken']) &&
				...
				if ($payment) {
					...
					if ($payment->status == 'APPROVED') {
						...
						wc_add_notice(sprintf(__('Your payment was declined for Afterpay Order #%s. Please try again. For more information, please contact the Afterpay Customer Service team on '.$this->assets['cs_number'].'.', 'woo_afterpay'), $payment->id), 'error');
		...
		private function render_placement($context, $product = null) {
			...
			$attributes['data-currency'] = get_option('woocommerce_currency');
			...
			if (isset($attributes['data-amount'])
